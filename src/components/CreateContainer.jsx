...
import { getAllFoodItems, saveItem, saveItems } from '../utils/firebaseFunctions';
...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    uploadTask.on('state_changed',(onSnapshot) => {
      const uploadProgress = (onSnapshot.bytesTransferred / onSnapshot.totalBytes) * 100;
    }, (error) => {
      console.log(error);
      setFields(true);
        ...
        <div className="group flex justify-center items-center flex-col border-2 border-dotted border-gray-300 w-full h-225 md:h-420 cursor-pointer rounded-lg">
          {isLoading ? ( <Loader /> ) : ( <>
              {!imageAsset ? ( <>
                <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer">
                  <div className="w-full h-full flex flex-col items-center justify-center cursor-pointer gap-2">
                    <MdCloudUpload className="text-gray-500 hover:test-gray-700" />
                    <p className="text-gray-500 text-3xl hover:test-gray-700">Click here to upload</p>
                  </div>
            ...
            <div className="relative h-full">
              ...
              />
              <button type="button" className="absolute bottom-3 right-3 p-3 rounded-full bg-red-500 text-xl cursor-pointer outline-none hover:shadow-lg duration-500 transition-all ease-in-out"
              onClick={deleteImage}
